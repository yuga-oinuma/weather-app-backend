...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'lambdas', 'api', 'get_weather_summary_daily'))

from lambdas.api.get_weather_summary_daily.app import lambda_handler
...
@patch('lambdas.api.get_weather_summary_daily.app.fetch_s3_json')
def test_lambda_handler_success(mock_fetch_s3_json, api_event):
    """正常なレスポンスのテスト"""
    mock_fetch_s3_json.return_value = '{"summary": "daily weather data"}'
    
    result = lambda_handler(api_event, None)
    
    assert result["statusCode"] == 200
    assert result["body"] == '{"summary": "daily weather data"}'
    mock_fetch_s3_json.assert_called_once_with(
        "test-bucket", 
        "analysis-results/summary/2024-01-01.json", 
        default="[]"
    )
...
@patch.dict(os.environ, {'BUCKET_NAME': 'test-bucket'})
@patch('lambdas.api.get_weather_summary_daily.app.fetch_s3_json')
def test_lambda_handler_empty_data(mock_fetch_s3_json, api_event):
    """データが空の場合のテスト"""
    mock_fetch_s3_json.return_value = "[]"
