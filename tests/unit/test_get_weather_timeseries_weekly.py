...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'lambdas', 'api', 'get_weather_timeseries_weekly'))

from lambdas.api.get_weather_timeseries_weekly.app import lambda_handler
...
def api_event():
    ...
    return {
        ...
        "httpMethod": "GET",
        "path": "/weather-timeseries-weekly",
        "headers": {},
        "body": None
    }
...
@patch('lambdas.api.get_weather_timeseries_weekly.app.fetch_s3_json')
def test_lambda_handler_success(mock_fetch_s3_json, api_event):
    """正常なレスポンスのテスト"""
    mock_fetch_s3_json.return_value = '{"timeseries": "weekly weather data"}'
    
    result = lambda_handler(api_event, None)
    
    assert result["statusCode"] == 200
    assert result["body"] == '{"timeseries": "weekly weather data"}'
    mock_fetch_s3_json.assert_called_once_with(
        "test-bucket", 
        "analysis-results/timeseries/weekly/2024-01-01.json", 
        default="[]"
    )
