...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'lambdas', 'scheduled', 'analyze_weather_weekly'))

from lambdas.scheduled.analyze_weather_weekly.app import lambda_handler
...
@patch('lambdas.scheduled.analyze_weather_weekly.app.boto3.client')
@patch('lambdas.scheduled.analyze_weather_weekly.app.get_summary')
@patch('lambdas.scheduled.analyze_weather_weekly.app.get_timeseries')
...
def test_lambda_handler_success(mock_print, mock_get_timeseries, mock_get_summary, mock_boto3_client, scheduled_event):
    ...
    mock_summary_data = {"summary": "test weekly summary data"}
    mock_timeseries_data = {"timeseries": "test weekly timeseries data"}
    ...
    assert "analysis-results/summary/weekly/" in summary_call[1]["Key"]
    ...
    assert "analysis-results/timeseries/weekly/" in timeseries_call[1]["Key"]
    ...
    assert any("Saved weekly summary data:" in call for call in print_calls)
    assert any("Saved weekly timeseries data:" in call for call in print_calls)